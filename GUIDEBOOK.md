...
7. **最終更新日の明示**: デプロイ情報セクションに「最終更新: 2026-04-23 - Vercel Serverless Functions対応完了」という更新履歴を追加し、Vercel Serverless Functions対応が完了したことを明示する（2026-04-23更新）
8. **Vercel設定の明文化**: モノレポ構成でのフロントエンド・バックエンドの個別デプロイ設定（Root Directory・Build Command・Output Directory・Install Command等）を明示し、Vercelプロジェクト設定時の参照資料として機能する（2026-04-23更新）
...
- **更新履歴確認時**: 最終更新日（2026-04-23）とVercel Serverless Functions対応完了の情報を確認し、プロジェクトの最新状態を把握するとき（2026-04-23更新）
...
**何を定義しているか:**
「どのステージを実行しどのステージをスキップするか」の根拠付き計画書です。変更インパクト評価・リスク評価・ワークフロー可視化・5ユニットの構成と実行順序が含まれます。

**なぜ存在するか:**
AI-DLC はすべてのステージを機械的に実行するのではなくプロジェクトの性質に応じて適応的に実行します。このファイルがあることで「なぜ User Stories をスキップしたか」「なぜ NFR Requirements をスキップしたか」という判断の根拠が明確になります。また、5ユニットの実行順序（shared → supabase → backend → frontend → ci-cd）の依存関係も記録されています。
...
### `VERCEL_DASHBOARD_SETUP.md`

**何を定義しているか:**
Vercel Dashboardでビルドコマンドを設定する手順を定義したクイックリファレンスガイドです。以下の内容が含まれます。

- **問題の原因**:
  - `vercel.json`の`buildCommand`が無視されている
...
- **ビルドプロセス**（3ステップ）:
  - ステップ1: `pnpm install` → すべての依存関係をインストール
  - ステップ2: `pnpm build`（ルートのpackage.json） → `pnpm --filter @monorepo/backend db:generate` → Prisma Clientを生成 → `pnpm --filter @monorepo/backend build:api` → `dist/api/index.js`を生成
  - ステップ3: `api/index.ts`を検出 → Serverless Functionとしてデプロイ
- **確認方法**:
  - デプロイ完了後、ビルドログで`✓ Prisma schema loaded from packages/backend/prisma/schema.prisma` / `✓ Generated Prisma Client`を確認
  - エンドポイントにアクセス: `curl https://kiro-project-backend-nine.vercel.app/api/health` → `{"status":"ok"}`
- **トラブルシューティング**（3つの問題）:
  - ビルドコマンドが実行されない: "Override"トグルをONにする → Build Commandを再入力 → Save
  - Prisma Clientエラーが続く: 環境変数を確認（`DATABASE_URL`（Pooler URL + `?pgbouncer=true`）/ `DIRECT_URL`（Direct URL））→ ビルドログで`prisma generate`が実行されているか確認 → 必要に応じてBuild Commandを`cd packages/backend && pnpm prisma generate && cd ../.. && pnpm build`に変更
  - TypeScript型エラー: `tsconfig.json`で`strict: false`に設定済み → 型エラーは警告として扱われ、ビルドは継続される
- **重要な注意事項**:
...
**なぜ存在するか:**
技術ブログアプリケーションのバックエンドをVercelにデプロイする際に、`vercel.json`の`buildCommand`が無視されてPrisma Clientが生成されないという問題が発生しました。以下の目的で作成されました:

1. **問題の原因の明確化**: `vercel.json`の`buildCommand`が無視される理由を明示し、開発者が問題の本質を理解できるようにする
2. **Vercel Dashboard設定の手順書**: Vercel Dashboardでビルドコマンドを設定する具体的な手順（Settings → General → Build & Development Settings → Framework Preset / Build Command / Output Directory / Install Command）を提供し、開発者が迷わず設定できるようにする
3. **ビルドプロセスの可視化**: `pnpm install` → `pnpm build` → Prisma Client生成 → APIビルド → Serverless Functionデプロイという3ステップのビルドプロセスを図解し、各ステップで何が実行されるかを明確にする
4. **確認方法の提供**: デプロイ完了後にビルドログで`✓ Generated Prisma Client`を確認する方法・エンドポイントにアクセスして`{"status":"ok"}`を確認する方法を提供し、デプロイ成功の判断基準を明示する
5. **トラブルシューティングの集約**: ビルドコマンドが実行されない・Prisma Clientエラーが続く・TypeScript型エラーという3つの典型的な問題について、原因と解決策を明示する
6. **vercel.json削除の理由の説明**: `vercel.json`を削除した理由（Dashboardの設定が優先されるため）を明示し、開発者が混乱しないようにする
7. **VERCEL_DEPLOYMENT.mdとVERCEL_FIX_FINAL.mdの補完**: `docs/VERCEL_DEPLOYMENT.md`は包括的なデプロイガイド、`VERCEL_FIX_FINAL.md`は`vercel.json`を使った修正ガイドだが、このドキュメントはVercel Dashboardでの設定に特化したクイックリファレンスとして機能する

**いつ使われるか:**

- **Vercelデプロイ時**: バックエンドをVercelにデプロイする際に、Vercel Dashboardでビルドコマンドを設定するとき（最優先で参照するドキュメント）
- **Prisma Clientエラー発生時**: デプロイ後に`Module '"@prisma/client"' has no exported member 'PrismaClient'`エラーが発生したとき
- **ビルドコマンドが実行されない時**: `vercel.json`にビルドコマンドを設定したのに実行されないとき
- **トラブルシューティング時**: ビルドログで`prisma generate`が実行されていない・TypeScript型エラーが発生するなどの問題が発生したとき
- **新規メンバーへのオンボーディング**: プロジェクトに参加した開発者が、Vercel Dashboardでのビルドコマンド設定方法を理解するとき
- **本番環境デプロイ時**: 本番環境へのデプロイ前に、Vercel Dashboardのビルド設定を確認するとき

**関連ファイル:**

- `docs/VERCEL_DEPLOYMENT.md` - Vercelデプロイの包括的なガイド（このドキュメントはVercel Dashboard設定に特化したクイックリファレンス）
- `VERCEL_FIX_FINAL.md` - `vercel.json`を使った修正ガイド（このドキュメントはVercel Dashboard設定を使った代替手段）
- `docs/VERCEL_ENV_VARS.md` - 環境変数の設定ガイド（Vercel Dashboardでの環境変数設定を含む）
...
**何を定義しているか:**
Vercelデプロイ時に発生する404エラーを解決するための修正ガイドです。以下の内容が含まれます。
...
- **修正内容**:
  - `vercel.json`の作成と設定（`version: 2` / `builds`セクションで`api/index.ts`を`@vercel/node`ビルダーで指定 / `routes`セクションですべてのリクエストを`/api/index.ts`にルーティング）
- **デプロイ手順**:
  - Gitコミット・プッシュ手順（`git add vercel.json` → `git commit -m "fix: Add vercel.json for proper routing"` → `git push origin main`）
- **期待される動作**:
  - `/api/health` → `api/index.ts` → `{"status":"ok"}` レスポンス
  - すべてのリクエストが`api/index.ts`にルーティングされる
- **確認方法**:
  - デプロイ後のヘルスチェック（`curl https://kiro-project-backend-nine.vercel.app/api/health` → `{"status":"ok"}`）

**なぜ存在するか:**
技術ブログアプリケーションのバックエンドをVercelにデプロイする際に、Prisma Clientは正常に生成されているにもかかわらず、エンドポイントが404エラーを返すという問題が発生しました。以下の目的で作成されました:

1. **404エラーの原因の明確化**: Prisma Clientの生成は成功しているが、Vercelが`api/index.ts`をServerless Functionとして認識していないことが原因であることを明示する
2. **vercel.jsonによるルーティング設定の提供**: `vercel.json`を作成し、`builds`セクションで`api/index.ts`を`@vercel/node`ビルダーで明示的に指定し、`routes`セクションですべてのリクエストを`/api/index.ts`にルーティングする設定を提供する
3. **デプロイ手順の明確化**: `vercel.json`作成後のGitコミット・プッシュ手順を明示し、開発者が迷わずデプロイできるようにする
4. **期待される動作の明示**: 修正後に`/api/health`エンドポイントが`{"status":"ok"}`を返すことを明示し、デプロイ成功の判断基準を提供する
5. **VERCEL_FIX_FINAL.mdとVERCEL_DASHBOARD_SETUP.mdの補完**: `VERCEL_FIX_FINAL.md`は500エラーの修正ガイド、`VERCEL_DASHBOARD_SETUP.md`はVercel Dashboardでのビルドコマンド設定ガイドだが、このドキュメントは404エラーに特化した修正ガイドとして機能する

**いつ使われるか:**

- **Vercelデプロイ時の404エラー発生時**: バックエンドをVercelにデプロイした後、エンドポイントが404エラーを返すとき（Prisma Clientは生成されているが、ルーティングが機能していない場合）
- **vercel.jsonのルーティング設定確認時**: `vercel.json`の`builds`セクションと`routes`セクションの設定を確認するとき
- **トラブルシューティング時**: Vercelデプロイ後にエンドポイントにアクセスできない・404エラーが発生するなどの問題が発生したとき
- **新規メンバーへのオンボーディング**: プロジェクトに参加した開発者が、Vercelのルーティング設定の仕組みを理解するとき
- **本番環境デプロイ時**: 本番環境へのデプロイ前に、`vercel.json`のルーティング設定を確認するとき

**関連ファイル:**

- `vercel.json` - Vercelのビルド・ルーティング設定（このガイドで作成される設定ファイル）
- `api/index.ts` - Vercel Serverless Functionsのエントリポイント（このガイドでルーティング対象となるファイル）
- `VERCEL_FIX_FINAL.md` - Vercelデプロイ時の500エラー修正ガイド（Prisma Client生成・TypeScriptビルド設定の修正）
- `VERCEL_DASHBOARD_SETUP.md` - Vercel Dashboardでのビルドコマンド設定ガイド（Dashboard設定を使った代替手段）
- `docs/VERCEL_DEPLOYMENT.md` - Vercelデプロイの包括的なガイド（フロントエンドとバックエンドの別々デプロイ戦略）
- `docs/VERCEL_TROUBLESHOOTING.md` - Vercelデプロイのトラブルシューティングガイド
- `packages/backend/src/app.ts` - Fastifyアプリケーションのエントリポイント（`api/index.ts`から呼び出される）
- `packages/backend/package.json` - バックエンドのビルドスクリプト（`build:api`スクリプトの定義）
- `README.md` - プロジェクト全体の説明書（Vercel対応の明示）
...
**何を定義しているか:**
Vercelデプロイ時にPrisma Clientが生成されない問題を解決するための最終修正手順を定義したクイックリファレンスガイドです。以下の内容が含まれます。

- **実施した修正**:
  - ルート`package.json`に`vercel-build`スクリプトを追加（`pnpm --filter @monorepo/backend db:generate`）
  - Vercelは`vercel-build`スクリプトを自動的に検出して実行する仕組みを利用
- **デプロイ手順**（3ステップ）:
  - ステップ1: 変更をコミット（`git add package.json` → `git commit -m "fix: Add vercel-build script for Prisma generation"` → `git push origin main`）
  - ステップ2: Vercelで自動デプロイ（GitHubにプッシュすると自動的に再デプロイ開始）
  - ステップ3: 動作確認（`curl https://kiro-project-backend-nine.vercel.app/api/health` → `{"status":"ok"}`）
- **期待されるビルドフロー**（3ステップ）:
  - ステップ1: `pnpm install` → 依存関係インストール
  - ステップ2: `pnpm vercel-build`（自動実行） → `pnpm --filter @monorepo/backend db:generate` → Prisma Client生成 → `node_modules/.prisma/client/`
  - ステップ3: `api/index.ts` → TypeScriptビルド → Serverless Functionとしてデプロイ
- **確認方法**:
  - デプロイ後のビルドログで`Running "pnpm vercel-build"` / `✓ Generated Prisma Client`を確認
  - エンドポイントにアクセス（`curl https://kiro-project-backend-nine.vercel.app/api/health` → `{"status":"ok"}`）
- **なぜこの方法が確実か**（3つの理由）:
  - Vercelの標準動作: `vercel-build`スクリプトは常に実行される
  - Dashboard設定不要: コードで管理できる（`vercel.json`や Dashboard の Build Command 設定が不要）
  - シンプル: 余計な設定ファイルが不要
- **それでも失敗する場合**:
  - 環境変数を再確認（Vercel Dashboard → Settings → Environment Variables: `DATABASE_URL`（必須）/ `DIRECT_URL`（必須）/ `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` / `CORS_ORIGIN` / `NODE_ENV=production`）
  - ビルドログを確認（Vercel Dashboard → Deployments → 最新のデプロイメントをクリック → "Building" セクションを展開 → `pnpm vercel-build`が実行されているか確認 → `Generated Prisma Client`メッセージを確認）

**なぜ存在するか:**
技術ブログアプリケーションのバックエンドをVercelにデプロイする際に、Prisma Clientが生成されないという問題が繰り返し発生しました。`vercel.json`の`buildCommand`設定や Vercel Dashboard の Build Command 設定を試みたものの、これらの方法では確実にPrisma Clientが生成されないケースがありました。以下の目的で作成されました:

1. **最もシンプルで確実な解決策の提供**: `vercel-build`スクリプトはVercelの標準動作として常に実行されるため、`vercel.json`の`buildCommand`設定や Dashboard の Build Command 設定よりも確実にPrisma Clientを生成できる。この最終的な解決策を明文化することで、同じ問題が再発した際に素早く解決できるようにする
2. **コードベースでの管理**: `vercel.json`や Dashboard の設定ではなく、ルート`package.json`の`vercel-build`スクリプトでPrisma Client生成を管理することで、設定がコードリポジトリに含まれ、チーム全員が同じ設定を共有できるようにする
3. **期待されるビルドフローの明示**: `pnpm install` → `pnpm vercel-build`（自動実行） → Prisma Client生成 → TypeScriptビルド → Serverless Functionデプロイという3ステップのビルドフローを図解し、各ステップで何が実行されるかを明確にする
4. **確認方法の提供**: デプロイ後にビルドログで`Running "pnpm vercel-build"` / `✓ Generated Prisma Client`を確認する方法・エンドポイントにアクセスして`{"status":"ok"}`を確認する方法を提供し、デプロイ成功の判断基準を明示する
5. **トラブルシューティングの集約**: それでも失敗する場合の2つの確認項目（環境変数の再確認・ビルドログの確認）を明示し、問題が解決しない場合の次のアクションを明確にする
6. **他のVercelデプロイドキュメントの補完**: `docs/VERCEL_DEPLOYMENT.md`（包括的なデプロイガイド）・`VERCEL_FIX_FINAL.md`（`vercel.json`を使った修正ガイド）・`VERCEL_DASHBOARD_SETUP.md`（Dashboard設定ガイド）という既存のドキュメントに対して、「最もシンプルで確実な方法」として`vercel-build`スクリプトを使った解決策を提供する

**いつ使われるか:**

- **Prisma Clientエラー発生時**: Vercelデプロイ後に`Module '"@prisma/client"' has no exported member 'PrismaClient'`エラーが発生したとき（最優先で参照するドキュメント）
- **ビルドコマンドが実行されない時**: `vercel.json`の`buildCommand`設定や Dashboard の Build Command 設定を試みたが、Prisma Clientが生成されないとき
- **最終的な解決策を探すとき**: 他の方法（`vercel.json`・Dashboard設定）を試したが解決しない場合に、最もシンプルで確実な方法として`vercel-build`スクリプトを試すとき
- **新規メンバーへのオンボーディング**: プロジェクトに参加した開発者が、Vercelデプロイ時のPrisma Client生成の仕組みを理解するとき
- **本番環境デプロイ時**: 本番環境へのデプロイ前に、Prisma Client生成が確実に実行される設定を確認するとき
- **トラブルシューティング時**: デプロイ後にビルドログで`pnpm vercel-build`が実行されているか・`Generated Prisma Client`メッセージが表示されているかを確認するとき

**関連ファイル:**

- `package.json` - ルートのpackage.json（`vercel-build`スクリプトを追加）
- `packages/backend/package.json` - バックエンドのpackage.json（`db:generate`スクリプトの定義）
- `packages/backend/prisma/schema.prisma` - Prismaスキーマ（Prisma Client生成の元となるファイル）
- `api/index.ts` - Vercel Serverless Functionsのエントリポイント
- `docs/VERCEL_DEPLOYMENT.md` - Vercelデプロイの包括的なガイド（このドキュメントはPrisma Client生成に特化した最終解決策）
- `VERCEL_FIX_FINAL.md` - `vercel.json`を使った修正ガイド（このドキュメントは`vercel-build`スクリプトを使った代替手段）
- `VERCEL_DASHBOARD_SETUP.md` - Dashboard設定ガイド（このドキュメントは`vercel-build`スクリプトを使った代替手段）
- `docs/VERCEL_ENV_VARS.md` - 環境変数の設定ガイド（Prisma Client生成に必要な`DATABASE_URL` / `DIRECT_URL`の設定を含む）
- `docs/VERCEL_500_ERROR_FIX.md` - バックエンドの500エラー修正ガイド（Prisma Client生成エラーが原因の500エラーを含む）
- `.kiro/specs/tech-blog/requirements.md` - 技術ブログ機能の要件定義（バックエンドAPIの要件）
- `.kiro/specs/tech-blog/design.md` - 技術ブログ機能の設計書（バックエンドアーキテクチャ・Fastify + Prisma + Supabase Auth）
- `aidlc-docs/construction/backend/code/summary.md` - バックエンドユニットの成果物一覧（Prisma Client生成の確認）
- `aidlc-docs/construction/build-and-test/build-instructions.md` - ビルド手順書（ローカルでのPrisma Client生成確認の参考）
- `README.md` - プロジェクト全体の説明書（Vercel対応を明示・デプロイ手順へのリンクを含む）
